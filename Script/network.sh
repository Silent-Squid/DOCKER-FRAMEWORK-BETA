...
# ---------------------------------------------------------- REDES POR VSWITCH ---------------------------------------------------------------------- #
switch="switch-red1"
sniffer_port="sniffer"

ovs-vsctl add-br $switch

ovs-vsctl -- add-port $switch $sniffer_port -- set interface $sniffer_port type=internal -- \
--id=@p get port $sniffer_port _uuid -- --id=@m create mirror name=wireshark output_port=@p select-all=true -- \
set bridge $switch mirror=@m

ip link set $sniffer_port up
